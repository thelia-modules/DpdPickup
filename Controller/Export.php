...
{
    /*************************************************************************/
    /* L'arrivée de Maitre Guigit détrône les anciens maitres pour           */
    /* corriger le soucis de json qui se supprime à chaque composer install  */
    /*                                                                       */
    ...
    public function exportfile()
    {
        ...
        foreach ($orders as $order) {
            ...
            if (false !== $collectionKey  && array_key_exists($collectionKey, $vform->getData()['order_ref_check'])
                && $vform->getData()['order_ref_check'][$collectionKey]
            ) {
                // Get if the package is assured, how many packages there are & their weight
                $assur_package = array_key_exists(
                    $collectionKey,
                    $vform->getData()['assur']
                ) ? $vform->getData()['assur'][$collectionKey] : false;
                // $pkgNumber = array_key_exists($collectionKey,
                // $vform->getData()['pkgNumber']) ? $vform->getData()['pkgNumber'][$collectionKey] : null;
                $pkgWeight = array_key_exists(
                    $collectionKey,
                    $vform->getData()['pkgWeight']
                ) ? $vform->getData()['pkgWeight'][$collectionKey] : null;
                ...
                if ($icirelais_code !== null) {
                    ...
                    // 1. Customer ref #1 = Order ref | MANDATORY
                    $res .= self::harmonise(
                        "",
                        'alphanumeric',
                        2
                    ...
                    // 5. Delivery name | MANDATORY
                    $res .= self::harmonise(
                        $address->getFirstname(),
                        'alphanumeric',
                        35
                    );
                    // 6. Delivery firstname
                    $res .= self::harmonise(
                        $address->getAddress2(),
                        'alphanumeric',
                        35
                    );
                    // 7. Delivery address 2
                    $res .= self::harmonise(
                        $address->getAddress3(),
                        'alphanumeric',
                        35
                    );
                    // 8. Delivery address 3
                    $res .= self::harmonise(
                        "",
                        'alphanumeric',
                        35
                    );
                    // 9. Delivery address 4 | SKIPPED
                    $res .= self::harmonise(
                        "",
                        'alphanumeric',
                        35
                    );
                    // 10. Delivery address 5 | SKIPPED
                    $res .= self::harmonise(
                        $address->getZipcode(),
                        'alphanumeric',
                        10
                    );
                    // 11. Delivery zipcode | MANDATORY
                    $res .= self::harmonise(
                        $address->getCity(),
                        'alphanumeric',
                        35
                    );
                    // 12. Delivery city | MANDATORY
                    $res .= self::harmonise(
                        "",
                        'alphanumeric',
                        10
                    ...
                    // 16. Delivery country code | MANDATORY
                    $res .= self::harmonise(
                        $address->getPhone(),
                        'alphanumeric',
                        20
                    ...
                    // 46. Consolidation number | SKIPPED
                    $res .= self::harmonise(
                        "",
                        'alphanumeric',
                        10
                    ...
                    // 55. Consolidation type | SKIPPED
                    $res .= self::harmonise(
                        "",
                        'alphanumeric',
                        2
                    );
                    // 56. Consolidation attribute | SKIPPED
                    $res .= self::harmonise(
                        "",
                        'alphanumeric',
                        1
                    ...
                    // 59. Contact name | SKIPPED
                    $res .= self::harmonise(
                        "",
                        'alphanumeric',
                        10
                    );
                    // 60. Digicode1 | SKIPPED
                    $res .= self::harmonise(
                        "",
                        'alphanumeric',
                        10
                    );
                    // 61. Digicode2 | SKIPPED
                    $res .= self::harmonise(
                        "",
                        'alphanumeric',
                        10
                    ...
                    if ($return_type != DpdPickup::RETURN_NONE) {
                        ...
                        // 68. Return address 2 | SKIPPED
                        $res .= self::harmonise(
                            "",
                            'alphanumeric',
                            35
                        );
                        // 69. Return address 3 | SKIPPED
                        $res .= self::harmonise(
                            "",
                            'alphanumeric',
                            35
                        );
                        // 70. Return address 4 | SKIPPED
                        $res .= self::harmonise(
                            "",
                            'alphanumeric',
                            35
                        );
                        // 71. Return address 5 | SKIPPED
                        $res .= self::harmonise(
                            $return_zipcode,
                            'alphanumeric',
                            10
